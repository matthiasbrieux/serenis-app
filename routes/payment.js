...
router.post('/create-checkout', express.json(), async (req, res) => {
  ...
  try {
    ...
    res.json({ error: 'Erreur paiement. Réessayez.' });
  }
});
...
router.get('/paiement-succes', async (req, res) => {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO sellers (uuid, email, password, pack, stripe_session_id, stripe_customer_id, paid_at)
      VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `).run(uuid, email.toLowerCase(), hashed, pack, session_id, session.customer || null);
    ...
    if (session.payment_status === 'paid') {
      ...
      if (!existing) {
        ...
        try {
          ...
          console.error('Webhook seller creation error:', e);
        }
      }
    }
  }
