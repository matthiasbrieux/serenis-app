...
app.get('/setup-test-vpm-2026', async (req, res) => {
  try {
    ...
    if (existing) {
      db.prepare('UPDATE sellers SET password=?, paid_at=CURRENT_TIMESTAMP, pack=?, contrat_signe=1 WHERE email=?').run(hashed, 'serenite', email);
      return res.send('<h2>✅ Compte mis à jour</h2><p>Email : test@venduparmoi.fr</p><p>Mot de passe : Test1234!</p><p><a href="/login">→ Se connecter</a></p>');
    }
    const uuid = uuidv4();
